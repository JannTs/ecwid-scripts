...
const BULK = {
  ...
  ${!same ? '• Оптова ціна визначається єдиним розміром партії, встановленим для<br>кожного виду Полісолу з асортименту в кошику<br>→ будь ласка: видаліть небажані позиції, що не відповідають цій вимозі.<br>' : ''}
  ${same && sum !== sizes[0] ? `• Загальна кількість банок: ${sum}, очікується: ${sizes[0]}<br>` : ''}
  ${same && extraItems ? '• У кошику є зайві товари<br>' : ''}
  ${same ? (
    sum < sizes[0]
