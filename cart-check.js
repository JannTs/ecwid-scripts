...
//Наразі партія ще не сформована, тому не можна розпочати оформлення
let lastAlertTime = 0;
...
const BULK = {
  ...
  notice.innerHTML = `
  ⚠️ <strong>Наразі партія ще не сформована, тому ще не можна розпочати оформлення:</strong><br>
  ${!same ? `
  ...
  ` : ''}
  ${(sum !== sizes[0]) ? `• Зараз кількість банок у кошику: ${sum}, очікується: ${sizes[0]}<br>` : ''}
